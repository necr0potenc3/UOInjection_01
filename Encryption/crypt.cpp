...
static const unsigned long p_box[18] =
{
...
static const unsigned long s_box[4 * 256] =
{
...
static const unsigned char g_key_table[CRYPT_GAMEKEY_COUNT][CRYPT_GAMEKEY_LENGTH] =
{
...
static const unsigned char g_seed_table[2][CRYPT_GAMESEED_COUNT][2][CRYPT_GAMESEED_LENGTH] =
{
    {
        {
...
void OldGameCrypt::init_tables()
{
    for(int key_index = 0; key_index < CRYPT_GAMEKEY_COUNT; key_index++)
    {
        ...
        const unsigned char * pkey = g_key_table[key_index];
        const unsigned char * pkey_end = g_key_table[key_index + 1];
...
void OldGameCrypt::DecryptFromServer(unsigned char * in, unsigned char * out, int len)
{
...
void OldGameCrypt::EncryptToClient(unsigned char * in, unsigned char * out, int len)
{
    // 2.0.0 did not use encryption
    if(in != out)
        memcpy(out,in,len);
...
void OldGameCrypt::EncryptToServer(unsigned char * in, unsigned char * out, int len)
{
    while(m_stream_pos + len > CRYPT_GAMETABLE_TRIGGER)
    {
        int len_remaining = CRYPT_GAMETABLE_TRIGGER - m_stream_pos;

        EncryptToServer(in, out, len_remaining);
...
void OldGameCrypt::DecryptFromClient(unsigned char * in, unsigned char * out, int len)
{
    while(m_stream_pos + len > CRYPT_GAMETABLE_TRIGGER)
    {
        int len_remaining = CRYPT_GAMETABLE_TRIGGER - m_stream_pos;

        DecryptFromClient(in, out, len_remaining);
