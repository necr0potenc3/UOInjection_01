...
void NormalCopier::operator () (char * dest, const char * src,
...
// This code originally based on part of UOX
...
CompressingCopier::CompressingCopier()
{
    initialise();
}

void CompressingCopier::initialise()
{
    bit_num = 0;
    out_data = 0;
}

inline bool CompressingCopier::output_bits(unsigned char * pdest,
    int & dest_index, int dest_size)
{
    while(bit_num >= 8)
    {
        // Buffer full.
        if(dest_index == dest_size)
            return false;
        bit_num -= 8;
        pdest[dest_index++] = (out_data >> bit_num) & 0xff;
    }
    return true;
}

void CompressingCopier::operator () (char * dest, const char * src,
    int & dest_size, int & src_size)
{
    ...
    // If the last call resulted in a full buffer, there is still data
    // left in out_data.
    if(!output_bits(pdest, dest_index, dest_size))
    {
    ...
    while(len--)
    {
        num_bits = bit_table[*psrc][0];
        bit_num += num_bits;
        assert(bit_num < 31);
        out_data = (out_data << num_bits) | bit_table[*psrc++][1];
        if(!output_bits(pdest, dest_index, dest_size))
        {
            dest_size = dest_index;
            src_size = psrc - src2;
...
bool CompressingCopier::flush(char * dest, int & dest_size)
{
    unsigned char * pdest = reinterpret_cast<unsigned char *>(dest);
    int dest_index = 0;
    int num_bits;
    ...
    bit_num += num_bits;
    out_data = (out_data << num_bits) | bit_table[256][1];
    if(!output_bits(pdest, dest_index, dest_size))
    {
        dest_size = dest_index;
        return false;
    ...
    {
        out_data <<= (8 - bit_num);
        pdest[dest_index++] = out_data & 0xff;
        bit_num = 0;
    }
